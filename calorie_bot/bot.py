...
# Telegram может присылать кнопки с вариационными селекторами, неразрывными
# пробелами или дополнительным текстом, поэтому шаблоны гибко ловят команды,
# иконки и ключевые фразы в любом сочетании.
LOG_DAY_PATTERN = r"(?is)^(/log_day\b.*|.*заполнить\W*день.*)$"
FINISH_DAY_PATTERN = r"(?is)^(/finish_day\b.*|.*завершить\W*день.*)$"
STATS_PATTERN = r"(?is)^(/stats\b.*|.*мой\W*прогресс.*)$"
PROFILE_PATTERN = r"(?is)^(/profile\b.*|.*мой\W*профиль.*)$"
